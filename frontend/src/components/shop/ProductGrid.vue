<template>
  <div>
    ...
    <div v-if="loading" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
      <div v-for="i in 8" :key="i" class="bg-white rounded-3xl overflow-hidden border border-cream-200">
        <div class="aspect-square skeleton"></div>
        <div class="p-6 space-y-3">
          <div class="h-4 w-3/4 skeleton"></div>
          <div class="h-3 w-full skeleton"></div>
          <div class="flex justify-between items-center pt-5 border-t border-cream-100">
    ...
    <div v-else class="text-center py-24">
      <div class="w-20 h-20 mx-auto mb-6 rounded-full bg-cream-50 flex items-center justify-center">
        <svg class="w-10 h-10 text-blush-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
        </svg>
      </div>
      <h3 class="text-xl font-display font-bold text-charcoal-900 mb-3">No products found</h3>
      <p class="text-sm font-sans text-charcoal-800/60">Try adjusting your search or filter criteria</p>
    </div>
  </div>
</template>
